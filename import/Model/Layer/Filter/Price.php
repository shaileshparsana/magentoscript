...
class Mage_Catalog_Model_Layer_Filter_Price extends Mage_Catalog_Model_Layer_Filter_Abstract
{
		   ...
		   $currentCategory = Mage::registry('current_category_filter');
            ...
            if (!$range) {
				...
				if (!$calculation || $calculation == self::RANGE_CALCULATION_AUTO) {
                    $index = 1;
                    do {
						$range = pow(10, (strlen(floor(100)) - $index));	
    ...
    protected function _renderRangeLabel($fromPrice, $toPrice)
    {
        ...
        if ($toPrice === '') {
			$finalPrice = $store->formatPrice($fromPrice+100);
            return Mage::helper('catalog')->__('%s - %s', $formattedFromPrice,$finalPrice );
		...
		/*$getPrice = Mage::app()->getRequest()->getParams();
		if($getPrice['price']){
		...
		$fromPrice = $fromPrice;
		$from = $currencySymbol.number_format($fromPrice, 0, '.', ',');
		$to = $currencySymbol.number_format($toPrice, 0, '.', ',');
        $formattedFromPrice  = $store->formatPrice($fromPrice);
		
        if ($toPrice === '') {
            $finalPrice = $fromPrice+100;
			$final = $currencySymbol.number_format($finalPrice, 0, '.', ',');
            return Mage::helper('catalog')->__('%s - %s', $from,$final );
        } elseif ($fromPrice == $toPrice && Mage::app()->getStore()->getConfig(self::XML_PATH_ONE_PRICE_INTERVAL)) {
            return $formattedFromPrice;
        } else {
            /*if ($fromPrice != $toPrice) {
			...
			if($from == '$')return Mage::helper('catalog')->__('$0 - %s', $to);		
    ...
    protected function _getItemsData()
    {
        ...
        if (!empty($dbRanges)) {
            ...
            foreach ($dbRanges as $index => $count) {
                ...
                $toPrice = ($index == $lastIndex) ? ($fromPrice+$range) : ($index * $range);
