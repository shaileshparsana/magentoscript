		...
		if((d_type == "object" && d_count > 0) || d_type == "string"){
			return this.each(function(x){
				...
				function processData(data, query){
					...
					//custom code to add entry for not exist barcodes from mobile devices	
				//	var scanid =jQuery.getUrlVar('scanid');
				//	var scanresult = jQuery.getUrlVar('ean');
					
					if(document.location.search.length)
					{
					var	iphonescancode =jQuery.getUrlVar('code');
					}
					if(input.hasClass('scanresultandorid') || iphonescancode=='scanbarcode' )
					{
						var current_time = 123;
						var scancode = query;
						
						var dataString = 'current_time='+ current_time + '&scancode=' + scancode;
				
						jQuery.ajax({
						type: "POST",
						url: actionUrl+'mobilescancodes',
						data: dataString,
