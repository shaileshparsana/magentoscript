	 ...
	 public function printAction() 
	   {
			...
			$ddate->getSelect()->reset(Zend_Db_Select::COLUMNS)->columns(array('ddate'=>'ddate','dtimetext'=>'dtimetext'))->joinRight(Mage::getSingleton('core/resource')->getTableName('ddate/ddate_store'),Mage::getSingleton('core/resource')->getTableName('ddate/ddate_store') . '.ddate_id = main_table.ddate_id',array('increment_id'=>'increment_id','is_gogreen'=>'is_gogreen','unattended_order'=>'unattended_order'));
			
			//$ddate->getSelect()->order(array('ddate',
             ///              'increment_id'));		   
		   
		   $ddate->getSelect()->order(array('ddate'));
		   $ddate->getSelect()->order("STR_TO_DATE(dtimetext, '%l:%i %p')");
